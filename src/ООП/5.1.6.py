...
class Rectangle:
    def __init__(self, a, b):
        ...
        if self.y == self.y2:
            self.length = round(math.sqrt((self.x2 - self.x) ** 2 + (self.y2 - self.y) ** 2), 2)
            self.width = round(math.sqrt((self.x1 - self.x) ** 2 + (self.y1 - self.y) ** 2), 2)
        else:
            self.xnew = self.x2
            self.ynew = self.y1
            self.length = round(math.sqrt((self.x1 - self.xnew) ** 2 + (self.y1 - self.ynew) ** 2), 2)
            self.width = round(math.sqrt((self.xnew - self.x2) ** 2 + (self.ynew - self.y2) ** 2), 2)
